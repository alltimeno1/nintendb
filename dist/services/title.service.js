...
async function findByTags(tags) {
    if (typeof tags === 'object') {
        ...
        const title = await title_model_1.Game.find({ tag: { $regex: reg } }, { description: 0 });
        return title;
    }
    const title = await title_model_1.Game.find({ tag: { $regex: tags } }, { description: 0 });
    return title;
}
...
async function updateWishItem(id, gameId) {
    ...
    if (bucket) {
        await bucket_model_1.Bucket.updateOne({ id }, { $addToSet: { list: gameId } });
    }
    else {
        await bucket_model_1.Bucket.create({ id, list: [gameId] });
    }
}
